class Camera {
  ...
  #extrudeCar(poly, carInfo, height = 15, wheelRadius = 5) {
    ...
    const carPolys = [...sides, ...ceilingParts];
    // console.log("carInfo.damaged in camera", carInfo.collision);
    if (!carInfo.collision) {
      for (const poly of carPolys) {
        // console.log("carInfo.damaged", carInfo.collision);
        poly.fill = "rgba(0,0,255,0.3)";
        poly.stroke = "blue";
      }
    } else {
      for (const poly of carPolys) {
        poly.fill = "rgba(255,0,0,0.3)";
        poly.stroke = "red";
      }
      // lastCarCollision = carInfo.collision;
    }

    const carWheelsPolys = [
      ...frontWheelLeftPolys,
      ...frontWheelRightPolys,
      ...backWheelLeftPolys,
      ...backWheelRightPolys,
    ];
    // carInfo.carWheelColor =

    for (const poly of carWheelsPolys) {
      if (!carInfo.controls.reverse) {
        poly.fill = "rgba(0,0,0,0.3)";
        poly.stroke = "black";
      } else {
        poly.fill = "rgba(255,0,0,0.7)";
      }
    }
  ...
  #getPolys(world) {
    ...
    for (const poly of buildingPolys) {
      poly.fill = "rgba(150,150,150,0.7)";
